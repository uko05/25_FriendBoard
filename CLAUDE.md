...
- さがす一覧の並び順は`latestSearchPosts`（`startSearchListener`のFirestoreクエリ
  ...
  しない＝登録・更新順（新しい方が上）。以前は「マッチ度」（`friendPreference`と
  相手の公開フィールドの一致度）で並べ替える`computeFriendMatch`（fields.js）が
  あったが、v12.6でバッジ表示を先に廃止した後も並び替え用途だけ残していた。
  v12.15で「登録・更新順に戻したい」という要望により並び替え自体を撤廃したのに
  伴い、完全に未使用になった`computeFriendMatch`関数も削除した。
- `matchesGenderPreference`（fields.js, v12.13追加）は、`friendPreference`の
  同性/異性/男女問わずの3項目**だけ**を対象にしたハードフィルター
  （`renderSearchList`の`.filter()`側に入っていて実際に一覧から消す。上記の
  「並び順」とは無関係の別機能）。判定は**双方向**：自分の希望×相手の性別、
  かつ相手の希望×自分の性別の両方を
